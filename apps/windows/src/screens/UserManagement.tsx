...
function formatAreaLabel(area?: string): string {
  ...
  if (normalized.startsWith("alma")) return "ALMACÉN";
  if (normalized.startsWith("admin")) return "ADMINISTRACIÓN";
  return String(area ?? "Sin área").toUpperCase();
}
...
export function UserManagement({
  ...
  const [areas, setAreas] = React.useState<AreaCatalogItem[]>([]);
  ...
  const [newCredentialType, setNewCredentialType] = React.useState<
    "password" | "pin"
  >("password");
  const [newRole, setNewRole] = React.useState<"admin" | "area">("area");
  const [newArea, setNewArea] = React.useState<UserArea>("");
  const [creating, setCreating] = React.useState(false);
  const [formError, setFormError] = React.useState<string | null>(null);
  const [newAreaName, setNewAreaName] = React.useState("");
  const [creatingArea, setCreatingArea] = React.useState(false);
  ...
  const [savingAreaName, setSavingAreaName] = React.useState(false);
  const [areaError, setAreaError] = React.useState<string | null>(null);
  const [areaSuccess, setAreaSuccess] = React.useState<string | null>(null);
  ...
  const [editArea, setEditArea] = React.useState<UserArea>("");
  const [editActivo, setEditActivo] = React.useState(true);
  const [editPassword, setEditPassword] = React.useState("");
  const [editCredentialType, setEditCredentialType] = React.useState<
    "password" | "pin"
  >("password");
  const [savingEdit, setSavingEdit] = React.useState(false);
  const [editError, setEditError] = React.useState<string | null>(null);
  const areaOptions = React.useMemo(
    () => areas.map((area) => ({ code: area.code, label: area.nombre })),
    [areas],
  );
  ...
  React.useEffect(() => {
    if (!newArea && areaOptions.length > 0) {
      setNewArea(areaOptions[0].code);
    }
  }, [areaOptions, newArea]);

  React.useEffect(() => {
    if (editing && editRole === "area" && !editArea && areaOptions.length > 0) {
      setEditArea(areaOptions[0].code);
    }
  }, [areaOptions, editArea, editRole, editing]);
  ...
  // Realtime: mantener last_seen como dato informativo cuando llega un heartbeat
  React.useEffect(() => {
      ...
      .on(
        ...
        (payload) => {
          ...
          setUsers((prev) =>
            prev.map((u) =>
              u.id === updated.id
                ? { ...u, lastSeen: updated.last_seen ?? undefined }
                : u,
            ),
          );
        },
  ...
  async function handleDelete(user: ManagedUser): Promise<void> {
    if (
      !window.confirm(
        `¿Eliminar el usuario "${user.username}"? Esta acción no se puede deshacer.`,
      )
    )
      return;
    try {
  ...
  function openEdit(user: ManagedUser): void {
    ...
    setEditArea(user.area ?? areaOptions[0]?.code ?? "");
  ...
  async function handleCreateArea(e: React.FormEvent): Promise<void> {
    e.preventDefault();
    setAreaError(null);
    setAreaSuccess(null);
    ...
    try {
      const created = await createArea(newAreaName);
      setNewAreaName("");
      await loadAreaCatalog();
      setNewArea(created.code);
      setAreaSuccess(`Área ${created.nombre} creada correctamente.`);
    } catch (err) {
      setAreaError(
  ...
  function startEditArea(area: AreaCatalogItem): void {
    setAreaError(null);
    setAreaSuccess(null);
  ...
  function cancelEditArea(): void {
    setEditingAreaId(null);
    setEditingAreaName("");
  }

  async function handleRenameArea(area: AreaCatalogItem): Promise<void> {
    setAreaError(null);
    setAreaSuccess(null);

    if (!editingAreaName.trim()) {
      setAreaError("Ingresa el nuevo nombre del área.");
      return;
    }

    setSavingAreaName(true);
    try {
      const updated = await renameArea(area.id, editingAreaName);
      await loadAreaCatalog();
      setAreaSuccess(`Área renombrada a ${updated.nombre}.`);
      cancelEditArea();
    } catch (err) {
      setAreaError(
        err instanceof Error ? err.message : "Error al renombrar el área",
      );
    } finally {
      setSavingAreaName(false);
    }
  }
  ...
  const normalizeUserKey = (value: string): string =>
    String(value ?? "")
      .trim()
      .toLowerCase();
  const isCurrentSessionUser = (user: ManagedUser): boolean => {
    ...
    return (
      Boolean(sessionId && rowId === sessionId) ||
      Boolean(sessionUsername && rowUsername === sessionUsername)
    );
  };
  const isUserVisibleActive = (user: ManagedUser): boolean =>
    user.activo && (onlineUserIds.has(user.id) || isCurrentSessionUser(user));
  const connectedNow = users.filter((u) => isUserVisibleActive(u));
  const connectedUsersOnPlatform = users
    .filter(
      (u) => u.activo && !isCurrentSessionUser(u) && isUserVisibleActive(u),
    )
    .sort((a, b) => {
      return String(b.lastSeen ?? b.lastSignInAt ?? "").localeCompare(
        String(a.lastSeen ?? a.lastSignInAt ?? ""),
      );
    });
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.areaCard}>
        <div>
          <h3 style={styles.formTitle}>Áreas de trabajo</h3>
          <p style={styles.areaHint}>
            Crea nuevas áreas y estarán disponibles para asignar a usuarios de
            rol Área.
          </p>
        </div>
        <form
          onSubmit={(e) => void handleCreateArea(e)}
          style={styles.areaForm}
        >
          <input
            style={styles.input}
            type="text"
            placeholder="Nombre del área (ej. Almacén)"
            value={newAreaName}
          ...
          >
            {creatingArea ? "Creando…" : "+ Agregar área"}
          </button>
        </form>
        <div style={styles.areaList}>
            ...
            <div key={area.id} style={styles.areaItem}>
              {editingAreaId === area.id ? (
                <>
                  ...
                  <button
                    type="button"
                    style={styles.areaActionSave}
                    onClick={() => void handleRenameArea(area)}
                    disabled={savingAreaName}
                  >
                    {savingAreaName ? "Guardando…" : "Guardar"}
                  </button>
                  <button
                    type="button"
                    style={styles.areaActionCancel}
                ...
                <>
                  <span style={styles.areaChip}>{area.nombre}</span>
                  <button
                    type="button"
                    style={styles.areaActionEdit}
          ...
          {areaOptions.length === 0 && (
            <span style={styles.areaEmpty}>No hay áreas registradas.</span>
          )}
        </div>
        {areaError && <p style={styles.errorText}>{areaError}</p>}
        {areaSuccess && <p style={styles.successText}>{areaSuccess}</p>}
      </div>
      ...
      <div style={styles.kpisRow}>
        <KpiCard
          ...
          value={String(connectedNow.length)}
          highlight
        />
      </div>
      ...
      <div style={styles.activePanel}>
        ...
        <div style={styles.activeInlineRow}>
          ...
          >
            <span
              style={{ ...styles.activeDot, ...styles.activeDotOnline }}
              title="Conectado"
            />
            <div style={styles.activeChipTextWrap}>
              <div
                style={{
                  ...styles.activeChipName,
                  ...styles.activeChipOnlineText,
                }}
              >
                Tú
              </div>
          ...
          {connectedUsersOnPlatform.map((u) => {
              ...
              >
                <span
                  style={{
                    ...styles.activeDot,
                    ...(connected
                      ? styles.activeDotOnline
                      : styles.activeDotOffline),
                  }}
                ...
                <div style={styles.activeChipTextWrap}>
                  <div
                    style={{
                      ...styles.activeChipName,
                      ...(connected ? styles.activeChipOnlineText : {}),
                    }}
                  >
                  ...
                  >
                    {connected ? "En línea ahora" : "Sin conexión"} ·{" "}
                    {u.platform}
                  </div>
                </div>
              </div>
          ...
          {connectedUsersOnPlatform.length === 0 && (
            <div style={styles.activeEmpty}>
              ...
              <tr key={u.id} style={styles.tr}>
                ...
                <td style={styles.td}>
                  <div style={styles.encargadoCell}>
                    <span style={styles.encargadoName}>
                      {u.fullName || "Sin nombre"}
                    </span>
                    <span style={styles.encargadoCargo}>
                      {formatCargo(u.role)}
                    </span>
                  </div>
                </td>
                ...
                <td style={styles.td}>
                  {u.role === "admin" ? "TODAS" : formatAreaLabel(u.area)}
                </td>
                <td style={styles.td}>
                  <span
                    style={{
                      ...styles.badge,
                      ...(u.activo ? styles.badgeActivo : styles.badgeInactivo),
                    }}
                  >
          ...
          <div style={styles.modalCard}>
            ...
            >
              ...
              <select
                ...
                value={editRole}
                onChange={(e) =>
                  setEditRole(e.target.value as "admin" | "area")
                }
              >
                <option value="admin">Administrador</option>
                <option value="area">Área</option>
              ...
              <input
                ...
                placeholder={
                  editCredentialType === "pin"
                    ? "4 dígitos (dejar vacío para no cambiar)"
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          ...styles.kpiValue,
          ...(highlight ? styles.kpiValueHighlight : {}),
        }}
      >
        {value}
      </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  areaHint: { margin: 0, fontSize: 13, color: "#475569" },
  areaForm: {
    display: "flex",
    gap: 10,
    flexWrap: "wrap",
  ...
  areaList: { display: "flex", gap: 8, flexWrap: "wrap" },
  areaItem: { display: "inline-flex", alignItems: "center", gap: 6 },
  areaChip: {
    padding: "6px 10px",
    borderRadius: 999,
    fontSize: 12,
  ...
  areaEmpty: { fontSize: 13, color: "#64748b" },
  formTitle: {
    fontSize: 15,
    fontWeight: 600,
    color: "#1a1a2e",
  ...
  activeDotOffline: {
    ...
    boxShadow: "0 0 0 3px rgba(148,163,184,.2)",
  },
  ...
  modalCard: {
    ...
    borderRadius: 12,
    padding: 20,
    boxShadow: "0 18px 45px rgba(17,24,39,0.28)",
  },
  modalTitle: {
    fontSize: 20,
    fontWeight: 700,
