...
function getAppIconPath() {
  if (require$$1$3.app.isPackaged) {
    const possiblePaths = [
      require$$1$1.join(process.resourcesPath, "app-icon.ico"),
      require$$1$1.join(__dirname, "../../app-icon.ico"),
      require$$1$1.join(require$$1$3.app.getAppPath(), "app-icon.ico")
    ];
    for (const p of possiblePaths) {
      try {
        require$$1.accessSync(p);
        return p;
      } catch (e) {
      }
    }
    ...
    return require$$1$1.join(process.cwd(), "public", "app-icon.ico");
  }
}
function createWindow() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    void win.loadFile(require$$1$1.join(__dirname, "../dist/index.html"));
  }
  return win;
}
function createUpdateOverlay(parent) {
  const overlay = new require$$1$3.BrowserWindow({
    width: 360,
    height: 140,
    frame: false,
    resizable: false,
...
<body>
  ...
  const area = require$$1$3.screen.getDisplayMatching(parent.getBounds()).workArea;
...
function updateOverlay(overlay, title, message, percent) {
...
function createUpdateReadyWindow(parent, version) {
  const updateWindow = new require$$1$3.BrowserWindow({
    width: 430,
    height: 245,
    frame: false,
    resizable: false,
...
function setupAutoUpdates(win) {
  ...
  main$1.autoUpdater.allowDowngrade = false;
  ...
  void main$1.autoUpdater.checkForUpdates();
...
require$$1$3.app.whenReady().then(() => {
  if (process.platform === "win32") {
    require$$1$3.app.setAppUserModelId("com.esmark.control");
  }
