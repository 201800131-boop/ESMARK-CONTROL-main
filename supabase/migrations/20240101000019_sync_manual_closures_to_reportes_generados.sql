-- Sincroniza cierres manuales con el historial que muestra la pantalla Reportes.
-- La UI lee public.reportes_generados, pero el cierre manual guardaba solo
-- public.ultimo_cierre_quincenal y public.cierres_quincenales.
...
select
  a.code,
  uc.realizado_por,
  coalesce(nullif(btrim(uc.realizado_por_nombre), ''), uc.realizado_por::text),
  uc.fecha_inicio,
  uc.fecha_fin
from public.ultimo_cierre_quincenal uc
cross join public.areas a
where a.code <> 'administracion'
  and not exists (
    select 1
    from public.reportes_generados rg
    where rg.area = a.code
      and rg.fecha_inicio = uc.fecha_inicio
      and rg.fecha_fin = uc.fecha_fin
      and rg.generado_por = uc.realizado_por
  );
...
begin
  if not exists (
    ...
    where id = p_realizado_por
      and rol = 'administracion'
  ) then
